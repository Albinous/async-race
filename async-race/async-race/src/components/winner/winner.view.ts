import { ICar, IWinner } from '../../core';

export abstract class WinnerView {
  public static getWinnerImage(winner: IWinner, car: ICar, pageNumber: number, index: number): string {
    const { wins, time } = winner;

    return `
      <td class="app-winner__number">${pageNumber * index}</td>
      <td class="app-winner__car">
        <svg
        version="1.1"
      ...
      <td class="app-winner__name">${car?.name}</td>
      <td class="app-winner__wins">${wins}</td>
      <td class="app-winner__time">${time}</td>
    `;
  }
}
